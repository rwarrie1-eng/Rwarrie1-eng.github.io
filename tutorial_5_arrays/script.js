...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    filterButton.addEventListener('click', (event) =>{
        ...
        // YOUR CODE HERE:
        const cheapRestaurants = restaurants.filter((restaurant) => {return restaurant.priceRange === '$' || restaurant.priceRange === '$$'; });
        filteredList.innerHTML = '';
        cheapRestaurants.forEach((restaurant)=> {filteredList.innerHTML += `<div>${restaurant.name} - ${restaurant.priceRange}</div>`;});
    ...
    mapButton.addEventListener('click', (event) => {
        ...
        // YOUR CODE HERE:
        const names = restaurants.map((restaurant) =>{ return restaurant.name; })
    ...
    findButton.addEventListener('click', (event) =>{
        ...
        // YOUR CODE HERE:
        const bestRestaurant = restaurants.find ((restaurant) => {return restaurant.rating === 4.8;    
        });
        foundItem.innerHTML = `<div>${bestRestaurant.name} - ${bestRestaurant.cuisine} - Rating: ${bestRestaurant.rating}</div>`;
